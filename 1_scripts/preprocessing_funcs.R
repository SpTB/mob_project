...
getDat <- function(exp_num = 'p1', type, include_files=c(), comments.txt=F) {
  
  # type: which type of data to preprocess: main, demo, tech or all
  # include_files: vector of which files to include; if empty -> include all
  # comments.txt: whether to save comments file when processing demo
  path_pattern = paste("0_data/", exp_num, sep='')
  fi=list.files(path=path_pattern, pattern='.csv', full.names = T)
  
  #only specific files
  if (!is_empty(include_files)) {
    fi = fi[include_files]
  }
  ...
  if (type=='main') {
    ...
    if (comments.txt) {
      write.table(all$describe, '2_pipeline/p1/comments.txt')
    }
