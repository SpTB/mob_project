OptimalThresholdsGen <- function(l1, sigma, Threshold, alpha = 1, v, B=10, l2=1,   n=11, perfBound=150, dy=0.005, tt=11){ #
  ...
  stepsOut <- ceiling(4*sigma*sqrt(dt)/dy) #Truncating a gaussian distribution at 4 times the standard deviation
  ...
  for (k in seq(n-1,1)){ #loop backwards through timesteps
   # print(k)
    ...
    givingUpIndex <- which(riskyExpReward[k,] > ((n-k)*l2*v*dt)^alpha)[1] #the point where the risky strategy expected reward becomes larger than the safe one 
    givingUpThreshold[k] <- -perfBound + dy*givingUpIndex + dy #convert the index to giving up threshold
    givingUpThreshold[k] <- max(givingUpThreshold[k],-perfBound+dy+2*stepsOut*dy)} #for computational reasons, we don't allow the threshold to be lower or too close to the lowest possible performance
 
   return(givingUpThreshold)
}

# optWrap <- function(pars) {
#   #wrapper to optimal threshold generator funcs
#   #contains parameters of secondary importance (simulation granularity, etc)
#  # args = unlist(list(...))
#   #args$a = 5
#   #unlist(args)
#  # print(args)
#   y = rlang::invoke(OptimalThresholdsGen, pars)
#   return (y)
# }

sim_wrap <- function (l1Vec, sigmaVec, thresholdVec, alphaVec, vVec, Bvec, l2=1, v=.25,  n=11, perfBound=150, dy=0.005, tt=11, ...) {
  
  #l1 = NULL# thresholdVec <- c(90,100,110)
  # sigmaVec <- c(5,10)
  # vVec <- c(.25,5)
  # alphaVec <- c(1)
  # n = 11
  # dt = 1
  # tt = 11
  combs<-expand.grid(l1Vec, thresholdVec, sigmaVec, vVec, alphaVec, Bvec) #all combinations of parameters
  
  optimalRiskyThresholds <- data.frame(goal = numeric(),
                                       lambda = numeric(),
                                       threshold = numeric(), 
                                       sigma = numeric(),
                                       safe_pay = numeric(), 
  ...
  for (i in seq(1:nrow(combs))){
    ...
    alpha <- comb[[5]]
    bonus <-comb[[6]]
    givingUpThreshold <- OptimalThresholdsGen(B=bonus, l1=lambda, sigma=sigma, v = safe, Threshold=threshold, alpha=alpha, ...)
    dummyDF <- data.frame(bonus = rep(bonus,n),
                          goal = rep(threshold,n),
                          #time = seq(dt,tt,dt), 
                          lambda = rep(lambda, n), 
                          sigma = rep(sigma,n),
                          alpha = rep(alpha, n),
                          safe_pay = rep(safe, n),
                          threshold = givingUpThreshold)
    optimalRiskyThresholds <- rbind(optimalRiskyThresholds, dummyDF)
    
    
  }
  optimalRiskyThresholds$step = rep(1:n, nrow(optimalRiskyThresholds)/n)
  
  optimalRiskyThresholds = optimalRiskyThresholds %>% filter(step<n) # filter out unnecessary last value
  #proc.time() - ptm  #end timer    
  return (optimalRiskyThresholds)
  
}


#write.csv(optimalRiskyThresholds, file = 'simData/optimalRiskyThresholdExperiment.csv')
